# bench-rig LM Studio — Historical Findings and Handoff

Status: Retained evidence for the provider-specific predecessor. It proves only
the exact bench-rig, LM Studio, model, configuration, and task lanes recorded
...
Controlled schema-v3 benchmarking and client qualification identify **Gemma 4 12B QAT Q4_0 as the qualified bench-rig winner**. It passed real OpenCode and Grok CLI repository tasks with tool use, targeted edits, and verification. Promoting it to a client default remains a separate configuration decision.
...
2. Promote `bench-rig-gemma4` in client configuration when desired; keep bench-rig unchanged.
...
Controlled tuning rejects both Qwen3.6 configurations and Qwen3 Coder for this GPU. Gemma is the qualified bench-rig winner after passing OpenCode and Grok tool-use validation. bench-rig remains Grok CLI's default.
